...
def init(cursor):
    """
    This function pulls the issues frequency data from MySQL (that was
    generated by global_init) into the process. Then, pandas is used to
    calculate the median number of issue events created per month for 
    each project, in that project's range of activity.
    """

    run = get_latest_run(cursor) # Use the latest run, from global_init()
