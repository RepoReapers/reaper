    ...
    def test_save(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                self.assertEqual(True, actual[4])
                self.assertEqual(9, actual[5])
                self.assertEqual(9.9, actual[6])
                self.assertEqual(9.9, actual[7])
                self.assertEqual(True, actual[8])
                self.assertEqual(9.9, actual[9])
                self.assertEqual(9.9, actual[10])
                self.assertEqual(99.99, actual[12])
            finally:
                self.database.post(
                    'DELETE FROM reaper_runs WHERE id = {0}'.format(run.run_id)
                )
